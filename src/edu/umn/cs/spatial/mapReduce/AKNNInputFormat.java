 ...
 *
 */
public class AKNNInputFormat extends MultiFileInputFormat<Rectangle, CollectionWritable<CollectionWritable<Point>>> {
	...
	@Override
	public RecordReader<Rectangle, CollectionWritable<CollectionWritable<Point>>> getRecordReader(InputSplit split,
			JobConf job, Reporter reporter) throws IOException {
	    reporter.setStatus(split.toString());
		@SuppressWarnings("unchecked")
		Class<RecordReader<Rectangle, CollectionWritable<Point>>> klass =
			(Class<RecordReader<Rectangle, CollectionWritable<Point>>>) PointsRecordReader.class
				.asSubclass(RecordReader.class);
	    //return new SpatialRecordReader(job, (FileSplit) split);
		return new MergeFileRecordReader<Rectangle, CollectionWritable<Point>>(
	...
	throws IOException {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			String blocks2readStr = job.get(AKNNInputFormat.BLOCKS2READ+'.'+i, "a");
