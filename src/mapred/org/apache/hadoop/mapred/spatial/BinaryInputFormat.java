...
public abstract class BinaryInputFormat<K extends Writable, V extends Writable>
  ...
  public InputSplit[] getSplits(final JobConf job, int numSplits) throws IOException {
    ...
    Vector<IndexedRectangle>[] spatialSplits = new Vector[inputFiles.length];
    ...
    for (int file_i = 0; file_i < inputFiles.length; file_i++) {
      spatialSplits[file_i] = new Vector<IndexedRectangle>();
          ...
          for (BlockLocation blockLocation : blockLocations) {
            if (blockLocation.getOffset() < fileSplit.getStart()
              ...
              if (blockLocation.getCellInfo() == null) {
                ...
                IndexedRectangle rect = new IndexedRectangle(splitIndex,
                    blockLocation.getCellInfo());
    ...
    SpatialAlgorithms.ResultCollector2<IndexedRectangle, IndexedRectangle> output = new SpatialAlgorithms.ResultCollector2<IndexedRectangle, IndexedRectangle>() {
      @Override
      public void add(IndexedRectangle r, IndexedRectangle s) {
        // Generate a combined file split for each pair of splits
        // Check that this pair was not taken before
        String thisPair = r.index+","+s.index;
        ...
        if (!takenPairs.contains(thisPair)) {
          ...
          FileSplit split1 = (FileSplit)splits[(int)r.index];
          FileSplit split2 = (FileSplit)splits[(int)s.index];
    ...
    for (int file_i = 0; file_i < heapSplits.length; file_i++) {
      for (int split_i : heapSplits[file_i]) {
        ...
        for (int file_j = file_i + 1; file_j < heapSplits.length; file_j++) {
          ...
          for (IndexedRectangle split_j : spatialSplits[file_j]) {
            combinedSplits.add(FileSplitUtil.combineFileSplits(job, (FileSplit)splits[split_i],
                (FileSplit)splits[(int)split_j.index]));
          }
        }
      }
    }
